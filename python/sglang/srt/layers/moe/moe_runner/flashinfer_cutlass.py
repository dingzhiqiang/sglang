...
class FlashInferCutlassMxfp4MoeQuantInfo(MoeQuantInfo):
    ...
    SM90 consumes W4A16-interleaved weights and scales. SM12x consumes packed
    MXFP4 weights and block-interleaved scales with MXFP8 activations.
    """

    # SM90 weights are interleaved; SM12x weights remain checkpoint-packed.
    ...
    # A non-None global scale selects the SM12x MXFP8 activation path.
    ...
    # Bailing clamps after SiLU and therefore needs FlashInfer's SwiGLU-step
    # variant. DeepSeek/GPT-OSS keep the original SwiGLU behavior.
...
def fused_experts_none_to_flashinfer_mxfp4(
    ...
    if TopKOutputChecker.format_is_bypassed(topk_output):
        topk_output = topk_output.to_standard(layer_id=runner_config.layer_id)
    topk_ids = topk_output.topk_ids
    topk_weights = topk_output.topk_weights
